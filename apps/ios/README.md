...
To create the repository's unsigned iOS 2.0.0 IPA from the command line, run this from the repository root:

    ./scripts/build-apple.sh ios

The artifact is written to `dist/apple/ios/MediaDownloader-iOS-2.0.0-unsigned.ipa`.
...
From the repository's `apps/ios` directory, run:
    ...
    cd apps/ios
    IOS_VERSION=2.0.0 IOS_BUILD_NUMBER=1 bash scripts/build-unsigned-ipa.sh

The output is:

    apps/ios/dist/MediaDownloader-iOS-2.0.0-unsigned.ipa

To copy it back from Windows PowerShell:

    scp cbzw025@REMOTE_MAC_IP:/path/to/tiktok-douyin-dl/apps/ios/dist/MediaDownloader-iOS-2.0.0-unsigned.ipa C:\Users\YOUR_NAME\Downloads\
