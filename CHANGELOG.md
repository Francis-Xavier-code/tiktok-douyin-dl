...
- **远程版本策略（停用旧版本）**：新增由 GitHub 托管的 `version-policy.json`，维护者可以在**不重新编译/重新发布各端安装包**的前提下，远程控制哪些旧版本被提示升级或拒绝运行。各端启动时拉取该文件（失败则放行，绝不影响正常使用），与自身版本号比对：
...
[Unreleased]: https://github.com/Francis-Xavier-code/tiktok-douyin-dl/compare/v1.7.0...HEAD
