#!/usr/bin/env python3
"""Install and verify the Playwright Chromium runtime.

Run with ``python -m media_downloader.browser.playwright``.
...
def verify_installation(target_exe: str):
    ...
    try:
        ...
        print("    然后重新运行: python -m media_downloader.browser.playwright")
        return False
...
╚══════════════════════════════════════════════════════════╝
    ...
    if success:
        ...
        print("    python apps/windows/gui/gui_downloader.py")
        print(f"{'='*60}\n")
        return True
    else:
        print(f"\n{'='*60}")
